...
import { CandidateDetails, TalentProfile } from '../utils/api';
...
export const DossierModal: React.FC<DossierModalProps> = ({
  ...
  const [activeSubTab, setActiveSubTab] = useState<'scorecard' | 'raw_dossier'>('scorecard');
      ...
      <div className="cyber-panel border-cyber-cyan/30 max-w-5xl w-full h-[90vh] overflow-hidden rounded-lg shadow-cyan-glow flex flex-col bg-cyber-dark/95">
        ...
        <div className="bg-cyber-gray/40 border-b border-cyber-slate/30 px-6 py-3 shrink-0 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() => setActiveSubTab('raw_dossier')}
              className={`px-3 py-1 rounded font-mono text-[10px] uppercase tracking-wider transition-colors border ${
                activeSubTab === 'raw_dossier'
                  ? 'bg-cyber-cyan/10 border-cyber-cyan text-cyber-cyan font-bold shadow-cyan-glow/20'
                  : 'bg-cyber-dark/50 border-cyber-slate/30 text-slate-400 hover:text-slate-200'
              }`}
            >
              ...
              <div className="grid grid-cols-1 gap-3">
                {profiles.map((profile) => {
                  ...
                  if (isAssessed) {
                    ...
                    if (hasRedFlags || assessment.match_score < 50) {
                  ...
                  return (
                    <div
                      key={profile.id}
                      className={`border rounded-lg transition-all duration-300 bg-cyber-gray/20 ${
                        isAssessed
                          ? isExpanded
                            ? 'border-cyber-cyan/40 bg-cyber-cyan/5'
                            : 'border-cyber-slate/40 hover:border-cyber-cyan/30'
                          : isRunning
                            ? 'border-cyber-cyan/50 bg-cyber-cyan/5 shadow-cyan-glow animate-pulse'
                            : 'border-cyber-slate/20 opacity-75'
                      }`}
                    >
                      {/* Row Brief Header */}
                      <div
            ...
            <div className="space-y-4 h-full flex flex-col min-h-[400px]">
              <div className="flex justify-between items-center pb-2 border-b border-cyber-slate/30 shrink-0">
                <h3 className="text-xs font-mono uppercase tracking-wider text-slate-300 flex items-center gap-1.5">
                  ...
                  <span>Sovereign Candidate Source Dossier Payload</span>
                </h3>
                <span className="px-2 py-0.5 bg-cyber-slate/40 text-slate-400 font-mono text-[9px] uppercase rounded">
              ...
              <div className="shrink-0 p-3.5 bg-cyber-gray/40 border border-cyber-slate/30 rounded-lg">
                <h4 className="text-[10px] font-mono uppercase tracking-wider text-slate-200 mb-1.5">Extracted Capabilities Ledger</h4>
